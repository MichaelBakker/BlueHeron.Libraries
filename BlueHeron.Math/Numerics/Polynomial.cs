...
namespace BlueHeron.Math.Numerics
{
	...
	public class Polynomial : IFormattable, IEquatable<Polynomial>, ICloneable
	{
		...
		public static Polynomial Add(Polynomial a, Polynomial b)
		{
			...
			var degree = System.Math.Max(a.Degree, b.Degree);
			var result = new double[degree + 1];

			var commonLength = System.Math.Min(System.Math.Min(ac.Length, bc.Length), result.Length);
			for (int i = 0; i < commonLength; i++)
			{
				result[i] = ac[i] + bc[i];
			}

			int acLength = System.Math.Min(ac.Length, result.Length);
			...
			int bcLength = System.Math.Min(bc.Length, result.Length);
		...
		public static Polynomial Add(Polynomial a, double b)
		{
			...
			var degree = System.Math.Max(a.Degree, 0);
			var result = new double[degree + 1];

			var commonLength = System.Math.Min(ac.Length, result.Length);
			for (int i = 0; i < commonLength; i++)
			{
				result[i] = ac[i];
			}
		...
		public static Polynomial PointwiseMultiply(Polynomial a, Polynomial b)
		{
			...
			var degree = System.Math.Min(a.Degree, b.Degree);
			var result = new double[degree + 1];
			for (int i = 0; i < result.Length; i++)
			{
				result[i] = ac[i] * bc[i];
		...
		{
			var hashNum = System.Math.Min(Degree + 1, 25);
			int hash = 17;
			unchecked
			{
